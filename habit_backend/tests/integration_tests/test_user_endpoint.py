...
def test_user_create_success(mock_application, user_url, valid_user, valid_user_id, valid_username, valid_user_email):
    ...
    assert create_user.user_id == valid_user_id
    assert create_user.username == valid_username
    assert create_user.email == valid_user_email
    assert response.status_code == 201


def test_get_user_by_username(mock_application, user_url, valid_user, valid_user_id, valid_username, valid_user_email):
    response = mock_application.post(url=user_url, json=dict(valid_user))

    response = mock_application.get(url=f"{user_url}/get_by_username/olta")
    olta_user = User(**response.json())

    assert olta_user.user_id == valid_user_id
    assert olta_user.username == valid_username
    assert olta_user.email == valid_user_email
    assert response.status_code == 200


def test_get_user_by_user_id(mock_application, user_url, valid_user, valid_user_id):
    response = mock_application.post(url=user_url, json=dict(valid_user))
...
def test_user_can_be_deleted(mock_application, user_url, valid_user_by_username_url, valid_user):
    response = mock_application.post(url=user_url, json=dict(valid_user))
